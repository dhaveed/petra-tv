<?php
if ( ! class_exists( 'DawnThemes_Controller' ) ) :

	class DawnThemes_Controller {
		...
		public function __construct() {
			...
			add_action('wp_ajax_dt_newsletter_ajax', array(&$this,'newsletter_ajax'));
			add_action('wp_ajax_nopriv_dt_newsletter_ajax', array(&$this,'newsletter_ajax'));
		}
		
		public function init(){
			if(isset($_REQUEST['action']) && $_REQUEST['action'] === 'dawnthemes_mailchimp_subscribe'){
		...
		public function facebook_init(){
			...
			if( !$user_ID ){
				...
				if( !$user_ID )
				{
					...
					if (dawnthemes_get_theme_option('facebook_login',0) == 0) {
			...
			if(get_option( 'users_can_register' )){
				...
				if ( is_wp_error( $errors ) ) {
					...
					$result = array(
						...
						'redirecturl'=>apply_filters('dawnthemes_register_redirect', home_url('/')),
					);
				}
			}else {
				$result = array(
		...
		protected function _register_new_user($user_login, $user_email, $user_password='', $cuser_password=''){
			...
			$user_id = wp_insert_user( apply_filters( 'dawnthemes_create_user_data', $new_user ) );
			...
			$headers = array();
			$from = dawnthemes_do_not_reply_address();
...
%4$s','dawnthemes');
		...
		protected function _mailchimp_subscribe($is_ajax = false) {
			$mailchimp_api = dawnthemes_get_theme_option( 'mailchimp_api', false );
			$success = -1;
			$message = '';
			...
			$data = $_REQUEST;
			$id = $_REQUEST['_mailchimp_id'];
			$nonce = $_REQUEST['_subscribe_nonce'];
			if(!wp_verify_nonce( $nonce, 'mailchimp_subscribe_nonce' )){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Verify Nonce Failed','dawnthemes') . '</span>' ));
				else
					wp_die(__('Verify Nonce Failed','dawnthemes'));
			}
			if(!filter_var(@$data['email'], FILTER_VALIDATE_EMAIL)){
				if($is_ajax)
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Please enter a valid email address','dawnthemes') . '</span>' ));
				else
					wp_die(__('Invalid Email','dawnthemes'));
			}
			if ( $mailchimp_api ) {
				...
				$list_id = dawnthemes_get_theme_option( 'mailchimp_list', 0 );
				...
				$mailchimp_group_name = dawnthemes_get_theme_option( 'mailchimp_group_name', '' );
				$mailchimp_group = dawnthemes_get_theme_option( 'mailchimp_group', '' );
				...
				$merge_vars = apply_filters('dawnthemes_mailchimp_groupings', $merge_vars);
				$double_optin = dawnthemes_get_theme_option( 'mailchimp_opt_in', '' ) === '1' ? true : false;
				$replace_interests = dawnthemes_get_theme_option( 'mailchimp_replace_interests', '' ) === '1' ? true : false;
				$send_welcome = dawnthemes_get_theme_option( 'mailchimp_welcome_email', '' ) === '1' ? true : false;
				
				try {
					$retval = $api->subscribe( 
						$list_id, 
						$email_address, 
						$merge_vars, 
						$double_optin);
					if(is_wp_error($retval)){
						if(!$is_ajax && current_user_can('administrator') && !empty($api->errorMessage)){
							wp_die($retval->get_error_message());
						}
					}else{
						$success = 1;
					}
			...
			if($is_ajax){
				if($success == 1){
					wp_send_json(array( 'success' => true, 'message' => '<span class="success-response"><i class="fa fa-check-circle"></i> ' . __('<strong>Well done!</strong> Subscribe to our Newsletter successful!','dawnthemes') . '</span>' ));
				}else{
					wp_send_json(array( 'success' => false, 'message' => '<span class="error-response"><i class="fa fa-times-circle"></i> ' . __('Can not subscribe our Newsletter','dawnthemes') . '</span>' ));
				}
			}else{
				$return = esc_url_raw(add_query_arg( 'mailchimp_subscribe',$success, wp_get_referer() ));
				wp_safe_redirect( $return.'#'.$id);
				die();
			}
		}
		...
		public function search() {
			...
			$output = "";
			$args = array(
				'post_type' => apply_filters('dawnthemes_ajax_search_form_post_type', 'any'), 
				'post_status' => 'publish', 
				'no_found_rows'       => true,
			...
			if( isset( $_REQUEST['tax'] ) && !empty( $_REQUEST['tax'] ) && isset( $_REQUEST['post_type'] ) ){
								
				$terms = get_terms($_REQUEST['tax'],array(
				...
				if ( count($terms) > 0 && count($terms) < 10 ){
					...
					foreach ( $terms as $term ) {
						//$output .= '<li><a href="' . esc_url( get_term_link( $term ) ) . '" title="' . esc_attr( $term->name ) . '">' . esc_html( $term->name ) . '</a></li>';
						...
						if( !empty($event_category_thumb_id) ){
							...
							$image = '<img src="'.dawnthemes_placeholder_img_src().'">';
						}
						$link = apply_filters( 'dawnthemes_search_item_url', get_term_link( $term->term_id, 'tribe_events_cat' ) );
						$output .= '<div class="search-entry">';
						$output .= '<div class="search-image"><a href="'.esc_url($link).'">' . $image . '</a></div>';
					...
					$output .= '</div>';
					$output .= '<div class="search-view-all"></div>';
				}else{
					$output .= '<div class="no-result">';
					$output .= __( 'No results matched!', 'dawnthemes' );
					$output .= '</div>';
				}
				
				$output = apply_filters('dawnthemes_ajax_search_form_result', $output);
				echo dawnthemes_echo($output);
				die();
			}
			
			$args = apply_filters( 'dawnthemes_search_query_args', $args );
			...
			if ( empty( $posts ) ) {
				...
				foreach ( $posts as $post ) {
					...
					$link = apply_filters( 'dawnthemes_search_item_url', get_permalink( $post->ID ) );
					$output .= '<div class="search-entry">';
					$output .= '<div class="search-image"><a href="'.$link.'">' . $image . '</a></div>';
					$output .= '<div class="search-content">';
					$output .= '<h4 class="search-title">';
					$output .= '<a href="' . $link . '">';
				...
				$output .= '</div>';
				$output .= '<div class="search-view-all"><a href="'.apply_filters('dawnthemes_ajax_viewall_link', home_url( '?s=' . $_REQUEST['s'] )).'">' .
					 __( 'View all results', 'dawnthemes' ) . '</a>';
			}
			$output = apply_filters('dawnthemes_ajax_search_form_result', $output);
			echo dawnthemes_echo($output);
