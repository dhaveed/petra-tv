...
class DawnThemes_Mobile_Detect
{
    /**
    ...
    */
	...
	function isRetina(){
		$retina = false;
		if ( isset( $_COOKIE['dawnthemes_retina'] ) ) {
			$ratio = $_COOKIE['dawnthemes_retina']; if( $ratio >= 2 ) { $retina = true; }
		}

		if(
			// Apple iOS 3.2-5.1 - Tested on the original iPad (4.3 / 5.0), iPad 2 (4.3), iPad 3 (5.1), original iPhone (3.1), iPhone 3 (3.2), 3GS (4.3), 4 (4.3 / 5.0), and 4S (5.1)
            $this->version('iPad')>=5.1 ||
            $this->version('iPhone')>=4.3 || $retina
		){
			return true;
		}
