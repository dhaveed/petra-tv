...
if ( ! class_exists( 'viem_posttype_viem_actor' ) ) {

	class viem_posttype_viem_actor {
		...
		public function __construct() {
			...
			if(is_admin()){
				...
				//Admin Columns
				add_filter( 'manage_edit-viem_actor_columns', array($this,'manage_edit_columns') );
				add_filter( 'manage_viem_actor_posts_custom_column',  array($this,'manage_custom_column'),10, 2 );
				
			}else{
				add_filter( 'template_include', array( $this, 'template_loader' ) );
				add_action( 'pre_get_posts', array( $this, 'pre_get_posts' ) );
				add_action( 'wp', array( $this, 'remove_actor_query' ) );
				add_action('viem_actor_info', array(__class__,'single_actor_info') );
				add_action('viem_actor_after_content', array(__class__,'get_related_video'), 10 );
				//add_action('viem_actor_after_content', array(__class__,'get_related_movie'), 20 );
			}
		...
		public function register_post_type() {
			if ( post_type_exists( 'viem_actor' ) )
				return;
			$permalinks = viem_get_theme_option( 'dt_theme_'.basename(get_template_directory()) . '_permalinks' );
			$actor_permalink = empty( $permalinks['viem_actor_base'] ) ? _x( 'actor', 'slug', 'viem-ultimate' ) : $permalinks['viem_actor_base'];
			
			register_post_type( 
				'viem_actor',
				apply_filters( 
					'viem_register_post_type_dt_actor', 
					array( 
						'labels' => array( 
							'name' => esc_html__( 'Actor', 'viem-ultimate' ), 
							'singular_name' => esc_html__( 'Actor', 'viem-ultimate' ), 
							'menu_name' => _x( 'Actor', 'Admin menu name', 'viem-ultimate' ), 
							'add_new' => esc_html__( 'Add Actor', 'viem-ultimate' ), 
							'add_new_item' => esc_html__( 'Add Actor', 'viem-ultimate' ), 
							'edit' => esc_html__( 'Edit', 'viem-ultimate' ), 
							'edit_item' => esc_html__( 'Edit Actor', 'viem-ultimate' ), 
							'new_item' => esc_html__( 'New Actor', 'viem-ultimate' ), 
							'view' => esc_html__( 'View Actor', 'viem-ultimate' ), 
							'view_item' => esc_html__( 'View Actor', 'viem-ultimate' ), 
							'search_items' => esc_html__( 'Search Actor', 'viem-ultimate' ), 
							'not_found' => esc_html__( 'No Actor found', 'viem-ultimate' ), 
							'not_found_in_trash' => esc_html__( 'No Actor found in trash', 'viem-ultimate' ), 
							'parent' => esc_html__( 'Parent Actor', 'viem-ultimate' ) ), 
						'public' => true, 
						'show_ui' => true, 
						'map_meta_cap' => true, 
						...
						'rewrite' => $actor_permalink ? array(
							'slug' => untrailingslashit( $actor_permalink ), 
							'with_front' => false, 
							'feeds' => true ) : false, 
						'has_archive' => ( $main_actor_page_id = viem_get_theme_option( 'main-actor-page') ) &&
							 get_post( $main_actor_page_id ) ? get_page_uri( $main_actor_page_id ) : 'actor', 
							'supports' => array( 'title', 'editor', 'author', 'thumbnail', 'comments') ) ) );
		}
		
		public function stop_redirect(){
			if ( is_singular('viem_actor') ) {
				global $wp_query;
		...
		public static function single_actor_info(){
			if( !is_singular('viem_actor') )
				return;
			?>
			<div class="single_actor_overview">
				<?php $nicknames = viem_get_post_meta('nickname');
				if( !empty($nicknames) ):
				?>
				<div class="actor-info"><span><?php echo esc_html__( 'Nickname:', 'viem-ultimate' );?></span> <?php echo esc_html($nicknames);?></div>
				<?php endif; ?>
				...
				if( !empty($born) ):
				?>
				<div class="actor-info"><span><?php echo esc_html__( 'Born:', 'viem-ultimate' );?></span> <?php echo date_i18n('M d, Y', strtotime($born) );?>
				...
				if( !empty($live_in) ):
				?>
				<div class="actor-info"><span><?php echo esc_html__( 'Lives in:', 'viem-ultimate' );?></span> <?php echo esc_html($live_in);?></div>
				<?php endif; ?>
				
				<?php $height = viem_get_post_meta('height');
				if( !empty($height) ):
				?>
				<div class="actor-info"><span><?php echo esc_html__( 'Height:', 'viem-ultimate' );?></span> <?php echo esc_html($height);?></div>
				<?php endif; ?>
			</div>
			<?php
		}
		...
		public function add_meta_boxes(){
			$meta_box = array (
				'id' => 'dt-metabox-viem_actor',
				'title' => esc_html__( 'Actor Info', 'viem-ultimate' ),
				'description' =>'',
				...
				'fields' => array (
					array (
						'label' => esc_html__( 'Nickname:', 'viem-ultimate' ),
						'name' => 'nickname',
						'type' => 'text',
						'width' => '350px',
						'placeholder' => '',
		...
		public function save_meta_boxes($post_id, $post){
			if('viem_actor'!==get_post_type($post))
				return;
		}		
		...
		public function template_loader($template){
			if(is_post_type_archive( 'viem_actor' ) ){
				$template = locate_template( 'archive-actor.php' );
			}
			return $template;
		}
		...
		public function pre_get_posts($q){
			...
			if ( ! $q->is_post_type_archive( 'viem_actor' ) && ! $q->is_tax( get_object_taxonomies( 'viem_actor' ) ) ) {
				return;
			}
			$this->actor_query($q);
			$this->remove_actor_query();
		}
		
		public function actor_query($q){
			$q->set( 'meta_query', $this->get_meta_query( $q->get( 'meta_query' ) ) );
			$q->set( 'tax_query', $this->get_tax_query( $q->get( 'tax_query' ) ) );
			do_action( 'viem_actor_query', $q, $this );
		}
		
		public function remove_actor_query() {
		...
		public function get_meta_query( $meta_query = array() ) {
			...
			return array_filter( apply_filters( 'viem_actor_query_meta_query', $meta_query, $this ) );
		}
		...
		public function get_tax_query( $tax_query = array() ) {
			...
			return array_filter( apply_filters( 'viem_actor_query_tax_query', $tax_query, $this ) );
		}
		...
		public static function get_current_filters() {
			if ( ! is_array( self::$_current_filters ) ) {
				...
				if ( $taxonomies = get_object_taxonomies( 'viem_actor' ) ) {
					foreach ( $taxonomies as $tax ) {
						$attribute    = str_replace('actor_', '', $tax);
						$taxonomy     = 'actor_'.$attribute;
						...
						self::$_current_filters[ $taxonomy ]['query_type'] = $query_type ? $query_type : apply_filters( 'viem_default_actor_filter_query_type', 'and' );
					}
				}
			}
		...
		public static function get_main_tax_query() {
			...
			if ( ! empty( $args['actor_cat'] ) ) {
				$tax_query[ 'actor_cat' ] = array(
					'taxonomy' => 'actor_cat',
					'terms'    => array( $args['actor_cat'] ),
					'field'    => 'slug',
				);
			}
		
			if ( ! empty( $args['actor_level'] ) ) {
				$tax_query[ 'actor_level' ] = array(
					'taxonomy' => 'actor_level',
					'terms'    => array( $args['actor_level'] ),
					'field'    => 'slug',
				);
			}
