	...
	videos:[
		{
			...
			vimeoID:"119641053",
			mp4:"//creativeinteractivemedia.com/player/videos/Tablet_Promo.mp4",
			imageUrl:"images/preview_images/poster2.jpg",
			imageTimer:4,
			prerollAD:"no",
			prerollGotoLink:"//codecanyon.net/",
			preroll_mp4:"//creativeinteractivemedia.com/player/videos/Short_Elegant_Logo_Reveal.mp4",
			prerollSkipTimer:5,
			midrollAD:"no",                                                                  
			midrollAD_displayTime:"00:10",                                                    
			midrollGotoLink:"//codecanyon.net/",                                         
			midroll_mp4:"//creativeinteractivemedia.com/player/videos/Logo_Explode.mp4", 
			midrollSkipTimer:5,	
			postrollAD:"no",                                                                
			postrollGotoLink:"//codecanyon.net/",                                        
			postroll_mp4:"//creativeinteractivemedia.com/player/videos/Logo_Light.mp4",  
			postrollSkipTimer:5,
			popupImg:"images/preview_images/popup.jpg",                        			  
			popupAdShow:"no",                                                                
			popupAdStartTime:"00:03",                                                         
			popupAdEndTime:"00:07",                                                          
			popupAdGoToLink:"//codecanyon.net/",
...
var Video = function(parent, options)
{
	...
	else //youtube single / channel / playlist
	{
		...
		if( (this.options.youtubePlaylistID != "" || this.options.youtubeChannelID != "") /*|| (this.options.videoType=="YouTube playlist")*/){
			...
			//var channelURL = '//gdata.youtube.com/feeds/api/users/'+youtubeChannelID+'/uploads?alt=json&orderby=published';
			...
			//===YOUTUBE PLAYLIST====//
			// var playListURL = '//gdata.youtube.com/feeds/api/playlists/'+youtubePlaylistID+'?v=2&alt=json';
    ...
    $(window).on('resize scroll', function() {
        ...
        if($(self.mainContainer.parent()).isInViewport()){
            ...
            if(self.initializedFloating){
                ...
                // Custom by DT
                $(self.mainContainer).parent().removeClass("viem_is_vp_sticky");
                if( self.mainContainer.parent().width() > 555 ){
                	self.mainContainer.find('.elite_vp_videoPlayer, .elite_vp_videoPlayerAD').width((self.mainContainer.parent().width())/(self.options.videoRatio));
                	self.mainContainer.find('.elite_vp_videoPlayer, .elite_vp_videoPlayerAD').height((self.mainContainer.parent().height())/(self.options.videoRatio));
            ...
            if(self.initializedFloating){
                ...
                $(self.mainContainer).parent().addClass("viem_is_vp_sticky");
                ...
                // Custom by DT
                if( self.mainContainer.parent().width() > 555 ){
                	var stickyMaxWidth = 555, stickyMaxHeight = 555/(self.options.videoRatio);
                	self.mainContainer.width(stickyMaxWidth);
                	self.mainContainer.height(stickyMaxHeight);
                	
                	self.mainContainer.find('.elite_vp_videoPlayer, .elite_vp_videoPlayerAD').width(stickyMaxWidth);
                	self.mainContainer.find('.elite_vp_videoPlayer, .elite_vp_videoPlayerAD').height(stickyMaxHeight);
...
Video.fn.createMutedBox = function(){
    ...
    this.mainContainer.append(this.mutedBox);
    ...
    this.mutedBox.append(this.mutedBoxIcon);
...
Video.fn.setupEvents = function()
{
    ...
    this.onended($.proxy(function()
    {
		...
		//play next on finish check
		 //next post on finish - Custom by DT
        if(self.options.onFinish=="Play next video" && this.options.youtubePlaylistID == ''){
        	this.ViemonNextVideoPost();
        }
...
Video.fn.ViemonNextVideoPost = function()
{
	var nextVideoURL;
	var viem_auto_next =  false;
	if( $('.viem-auto-next-op').length && $('.viem-auto-next-op').hasClass('active')){
		viem_auto_next = true;
	}
	if(viem_auto_next){
		if( $('.viem-playlist-panel').length ){
			var currentVideo = $('.viem-playlist-panel').find('.viem-playlist-items .viem-playlist-item-video.active');
			if( currentVideo.next().length > 0 ){
				nextVideoURL = currentVideo.next().find('.item-video-title').find('a').attr('href');
			}else{
				nextVideoURL = $('.viem-playlist-panel').find('.viem-playlist-items').eq(0).find('.item-video-title').find('a').attr('href');
			}

		}else if( $('.viem-series-panel').length ){
			var currentVideo = $('.viem-series-panel').find('.viem-series-items .viem-playlist-item-video.active');
			if( currentVideo.next().length > 0 ){
				nextVideoURL = currentVideo.next().find('.item-video-content').find('a').attr('href');
			}else{
				nextVideoURL = $('.viem-series-panel').find('.viem-series-items').eq(0).find('.item-video-content').find('a').attr('href');
			}
		}else if( $('.viem-video-nav').length ){
			nextVideoURL = $('.viem-video-nav').find('a.next-video').attr('href');
			
		}
		if( typeof(nextVideoURL)!='undefined' && nextVideoURL != null ){
