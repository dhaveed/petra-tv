<?php
if ( ! class_exists( 'DT_Category_Dropdown' ) ) :

	class DT_Category_Dropdown extends Walker {
...
function dt_walk_category_dropdown_tree() {
	$args = func_get_args();
	// the user's options are the third parameter
	if ( empty( $args[2]['walker'] ) || ! ( $args[2]['walker'] instanceof Walker ) ) {
		$walker = new DT_Category_Dropdown;
	} else {
...
if ( ! class_exists( 'DT_Walker' ) ) :

	class DT_Walker extends Walker_Nav_Menu {
...
if ( ! class_exists( 'DT_Underline_Walker' ) ) :

	class DT_Underline_Walker extends DT_Walker {
...
if ( ! class_exists( 'DT_Mega_Walker' ) ) :

	class DT_Mega_Walker extends DT_Walker {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $depth === 0 ) {
				$this->_megamenu_status = get_post_meta( $item->ID, '_dt_megamenu_status', true );
				$this->_megamenu_fullwidth = get_post_meta( $item->ID, '_dt_megamenu_fullwidth', true );
				$this->_megamenu_columns = get_post_meta( $item->ID, '_dt_megamenu_columns', true );
			...
			$this->_megamenu_title = get_post_meta( $item->ID, '_dt_megamenu_title', true );
			$this->_menu_icon = get_post_meta( $item->ID, '_dt_menu_icon', true );
			$this->_megamenu_sidebar = get_post_meta( $item->ID, '_dt_megamenu_sidebar', true );
			$this->_menu_visibility = get_post_meta( $item->ID, '_dt_menu_visibility', true );
		...
		public function end_el( &$output, $item, $depth = 0, $args = array() ) {
			$output .= "</li>\n";
			$header_style = viem_get_theme_option('header-style','below');
			if($header_style =='center2'){
				static $static_nav_count = 0;
				if($depth === 0){
					$logo_url = viem_get_theme_option('logo');
					$logo_fixed_url = viem_get_theme_option('logo-fixed','');
					$logo_transparent_url = viem_get_theme_option('logo-transparent','');
					$logo_mobile_url = viem_get_theme_option('logo-mobile','');
					if(empty($logo_fixed_url))
						$logo_fixed_url = $logo_url;
					if(empty($logo_mobile_url))
						$logo_mobile_url = $logo_url;
					$menu_transparent = viem_get_theme_option('menu-transparent',0);
					if($menu_transparent)
