jQuery(document).ready(function($){
	...
	function processMegaAtts(){
		$( '.wpmega-atts.wpmega-unprocessed' ).each( function(){
			...
			var $inputs = $( this ).find( ':input:not( .megamenu_options_input )' );
			...
			$( this ).find( '.megamenu_options_input' ).val( options );
	...
	$megaInputs.live( 'change' , function(){
		...
		var $inputs = $attGroup.find( ':input:not( .megamenu_options_input )' );
		$inputs.each( function(){
			$( this ).attr( 'name' , $( this ).attr( 'data-name' ) );
		});
		var options = $inputs.serialize();
		console.log( options );
		$attGroup.find( '.megamenu_options_input' ).val( options );
		$inputs.removeAttr( 'name' );
	});
	...
	if(typeof wpNavMenu != 'undefined'){
		wpNavMenu.addItemToMenu = function(menuItem, processMethod, callback) {
			...
			params = {
				"action": "dt_megamenu_add_menu_item",
				"menu": menu,
	...
	$('#wp-mega-menu-navlocs-submit').on( 'click' ,function(){
