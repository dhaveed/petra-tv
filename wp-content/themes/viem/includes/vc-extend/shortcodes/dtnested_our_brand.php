...
vc_map(
	array(
		'base' => 'dtnested_our_brand',
		'name' => esc_html__( 'Our Brand', 'viem' ),
		"category" => esc_html__( "DawnThemes", 'viem' ),
		'description' => esc_html__( 'Our Brand with slider', 'viem' ),
		'class' => 'dt-vc-element dt-vc-element-dtnested_our_brand',
		'icon' => 'dt-vc-icon-dtnested_our_brand',
		'show_settings_on_create' => true,
		'is_container' => true,
		'js_view' => 'DTVCOurBrand',
		'params' => array(
			array(
				'type' => 'textfield',
...
<div class="wpb_tabs_holder wpb_holder clearfix vc_container_for_children">
	<ul class="tabs_controls">
		...
		'default_content' => '
[dtsingle_our_brand_item title="' . esc_html__( 'Item 1', 'viem' ) . '" tab_id="' . time() . '-1-' . rand( 0, 100 ) . '"][/dtsingle_our_brand_item]
[dtsingle_our_brand_item title="' . esc_html__( 'Item 2', 'viem' ) . '" tab_id="' . time() . '-2-' . rand( 0, 100 ) . '"][/dtsingle_our_brand_item]
[dtsingle_our_brand_item title="' . esc_html__( 'Item 3', 'viem' ) . '" tab_id="' . time() . '-3-' . rand( 0, 100 ) . '"][/dtsingle_our_brand_item]
					  ' ) );

vc_map(
	array(
		'name' => esc_html__( 'Our Brand Item', 'viem' ),
		'base' => 'dtsingle_our_brand_item',
		'allowed_container_element' => 'vc_row',
		'is_container' => true,
		'content_element' => false,
		"category" => esc_html__( "DawnThemes", 'viem' ),
		'js_view' => 'DTVCOurBrandItem',
		'params' => array(
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Title', 'viem' ),
				'param_name' => 'title',
				'description' => esc_html__( 'Title.', 'viem' ) ),
			array(
				'type' => 'attach_image',
				'heading' => esc_html__( 'Logo', 'viem' ),
				'param_name' => 'image',
				'description' => esc_html__( 'Brand Logo.', 'viem' ) ),
			array(
				'type' => 'dropdown',
				'std' => 'default',
				'heading' => esc_html__( 'Logo Size', 'viem' ),
				'param_name' => 'img_size',
			...
			array(
				'type' => 'checkbox',
				'heading' => esc_html__( 'Use rel="nofollow" for link', 'viem' ),
				'param_name' => 'use_nofollow',
...
class WPBakeryShortCode_DTnested_Our_Brand extends WPBakeryShortCode_VC_Tabs {
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		preg_match_all(
			'/dt_brand_item title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i',
			$content,
			$matches,
			PREG_OFFSET_CAPTURE );
	...
	public function getTabTemplate() {
		return '<div class="wpb_template">' .
			do_shortcode( '[dtsingle_our_brand_item title="' . DTVC_ITEM_TITLE . '" tab_id=""][/dtsingle_our_brand_item]' ) .
			'</div>';
	}
}


class WPBakeryShortCode_DTSingle_Our_Brand_Item extends WPBakeryShortCode {
	/**
	 * Find html template for shortcode output.
	 */
	protected function findShortcodeTemplate() {
